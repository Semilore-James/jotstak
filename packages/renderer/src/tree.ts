...
export interface Size {
  /** Natural width in CSS px at full size, widened by the estimate's safety margin. */
  w: number;
  /** Natural height in CSS px — always a whole number of rows. */
  h: number;
}
...
export type Placement = { width: "column" | "full"; page: "portrait" | "landscape"; tall: boolean };
...
export function placeTree(look: Look, size: Size, pinned: string, n: BlockNode, diagnostics: Diagnostic[]): Placement {
  ...
  else if (look === "outline" || size.w <= column) placement = { width: "column", page: "portrait", tall: false };
  ...
  if (look !== "outline") {
    if (pinned === "auto" && placement.page === "landscape" && size.w <= land) {
      note("info", `This tree is about ${px(size.w)} wide — more than a portrait page (${px(content)}) — so it prints on a landscape page of its own. On screen it is shown scaled to fit.`);
    } else if (size.w > room) {
      ...
      if (scale >= MIN_SCALE_BEFORE_LANDSCAPE) {
        ...
        const hint = look === "columns" ? "Fewer levels or shorter labels would fit." : "dir=right grows down the page instead of across it.";
        note(
          pinned === "auto" ? "warning" : "info",
          `This tree is about ${px(size.w)} wide, more than the ${px(room)} it has, so it is scaled to ${pct}% to fit. ${hint}`,
        );
      }
    }
  }
  const rowsPerPage = placement.page === "landscape" ? PAGE.landscape.rows : PAGE.portrait.rows;
  placement.tall = size.h > rowsPerPage * TREE.row;
  return placement;
}
